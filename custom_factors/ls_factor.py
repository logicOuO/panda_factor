...
class LSFactor(BaseCustomFactor):
    ...
    def calculate(self, factors):
        ...
        amt_z = (amount - self.MA(amount, 20)) / (self.STD(amount, 20) + 1e-4)
        range_z = (range_ratio - self.MA(range_ratio, 20)) / (self.STD(range_ratio, 20) + 1e-4)
        ls_s = amt_z - range_z
        ...
        ls_i = (illiq_a > (self.MA(illiq_a, 20) + 2 * self.STD(illiq_a, 20))).astype(float)

        turn1 = volume / (capital + 1e-4)
        ls_t = (turn1 > (self.MA(turn1, 20) + 2 * self.STD(turn1, 20))).astype(float)
